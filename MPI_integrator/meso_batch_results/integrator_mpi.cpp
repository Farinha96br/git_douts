...
int main(int argc, char** argv) {
   MPI_Init(&argc, &argv);
   int gencase = 0; // 0 = grid de pontos, 1 = pontos aleatorios
   ...
   if (rank == 0 ){
      ...
      if (gencase == 0){
         int NX = 128; // numero de pontos em X
         int NY = 128; // numero de pontos em Y
         double dx = (2.0*M_PI/3 - 0.0)/(NX-1.0); // faz o passo usando a ideia do linspace
         double dy = (2.0*M_PI/3 - 0.0)/(NY-1.0); // 
   ...
   double tau = abs(2.0*M_PI/(v*ky[1])); // usado p mapas
	step = tau/100.0; // passo eh um milesimo do periodo da perturbação;
