...
if saveSteps:
    ...
    ax[0,1].pcolormesh(bb, aa, dil, cmap = "Greys_r")
    ax[0,2].pcolormesh(bb, aa, cls, cmap = "Greys_r")
    ax[1,0].pcolormesh(bb, aa, ero, cmap = "Greys_r")
    ax[1,1].pcolormesh(bb, aa, op, cmap = "Greys_r")
...
#    
    ax[1,2].text(cy[i],cx[i],str(i),c = "black",size="xx-small")
    pointfile.write(str(cx[i]) + "\t" + str(cy[i]) + "\n")

ax[1,2].scatter(cy,cx,s = 0.2,c = "black")

plt.savefig(folder + "/mm_steps",bbox_inches='tight',dpi = 900) # salva em png

os.makedirs("r_steps",exist_ok=True)
plt.savefig(folder + "/mm_steps.png",bbox_inches='tight',dpi = 300) # salva em png
plt.savefig("r_steps/" + folder[-17:] + ".png",bbox_inches='tight',dpi = 300) # salva em png
