...
void mysyst( const state &x , state &dxdt , double t ){
...
// Definindo a função observadora
int strobe_c = 0; // contator pro esquema de estrobo que esotu usando
void write_mine( const state &x , double t){   
...
{   
    ...
    // define o estrobo
    //strobe = abs(2.0*M_PI/((w[1]/kx[1] - w[0]/kx[0])*kx[1])); // usado p mapas
    strobe = 0.1; // Usado pra ver a trajetoria da particula em sí

    double tf = iterations*strobe; // calcula o tempo final dado as N iteraçoes do estrobo
    state x = {x0, y0}; // initial conditions
    state x2 = {x0 + 1e-7, y0 + 1e-7}; // initial conditions
    runge_kutta4 < state > rk; // formato de stepper/passo/integraçao do integrador
    ...
    if (n1 == 0){
        ...
        fprintf(reportfile, "#i \t A \t w \t kx \t ky \t phasey\n");
        fprintf(reportfile, "1 \t %lf \t %lf \t %lf \t %lf \t %lf \n",A[0],w[0],kx[0],ky[0],phasey[0]);
        fprintf(reportfile, "1 \t %lf \t %lf \t %lf \t %lf \t %lf \n",A[1],w[1],kx[1],ky[1],phasey[1]);
        fclose(reportfile);
    }
    double D;
    double l;
    while (t <= tf){
        if ((t > strobe_c*strobe - step/2) && (t < strobe_c*strobe + step/2)){
            D = sqrt(pow(x[0]-x2[0],2) + pow(x[1]-x2[1],2));
            l = log(D)/t;
            fprintf(outfile,"%lf \t %lf \t %lf \t %lf \t %lf \t %lf \n",t,x[0],x[1],x2[0],x2[1],l);
            strobe_c++;
        } 
        
        rk.do_step(mysyst, x, t,  step);
        rk.do_step(mysyst, x2, t, step);  
        t += step;
    }
